...
## Simulation Visuals (important — not in the Tech Transfer doc)

The Tech Transfer doc explicitly puts simulation canvas rendering out of scope and only specifies the **frame** around it (`.sim-canvas-wrap`, `.fig-caption`). But the prototype renders beautifully styled SVG scenes for several sims — that's the look you want inside those frames.

Those scenes live as React components in the JSX references:

- **`references/lib/shared.jsx`** — contains `PhotoelectricScene` (dark viewport · caesium cathode · amber photon rays · blue ejected electrons · JetBrains Mono labels) and `KEGraph` (KE vs ν with threshold marker). These are the canonical visual targets for the photoelectric sim.
- **`references/lib/dir-a-v2.jsx`** — contains the full prototype for Home, Photoelectric, Bohr, Orbitals, and IR Spectra screens, showing how those scenes compose with the page chrome.

### How to apply these to the real codebase

The prototype scenes are SVG; your sims render to `<canvas>` via `.ts` files like `photoelectric.ts`. Two valid paths per sim:

1. **Translate to canvas draw calls.** Read the SVG in `PhotoelectricScene` and port each primitive (rects for cathode, lines+circles for photons, circles+lines for electrons, text labels) to equivalent `ctx.fillRect` / `ctx.beginPath` calls in `photoelectric.ts`. Drive positions from existing simulation state. This keeps all sim logic intact.
2. **Mount the SVG and drive it.** If the sim doesn't need pixel-level canvas (e.g. `photoelectric.ts` is mostly static chrome + a few animated elements), swap the `<canvas>` for an inline SVG and update element attributes from your existing event handlers.

**Colors to preserve exactly** (pulled from `PhotoelectricScene`):
- Dark viewport bg: `#0a0612` (`--canvas-bg`)
- Photons: `#c8892a` (`--amber`)
- Ejected electrons: `#7cc4ff`
- Cathode plate: `#8a8f99` with `#cfd3dc` highlight stripe
- Grid lines: `rgba(255,255,255,0.05)`
- Labels: `rgba(255,255,255,0.5)` — but per Tech Transfer § 9 gotcha, swap JetBrains Mono → Lato 600 + tabular-nums + wide tracking

**Scope note:** This is the one place where you'll touch `.ts` files despite the Tech Transfer's "no sim code changes" principle. It's a conscious exception — the visual upgrade *is* the point. Keep the simulation **math and physics** untouched; only the rendering primitives change.

### Prompt addition for Claude Code

Add this to your kickoff prompt:
> When you reach the simulation shells, read `references/lib/shared.jsx` and `references/lib/dir-a-v2.jsx` — those show the target visuals for photoelectric, Bohr, orbitals, and IR spectra. Translate each SVG scene into equivalent canvas or inline-SVG rendering inside the existing `.ts` files, preserving simulation math but upgrading the visual layer.
...
└── references/
    ├── Tech Transfer.html             ← THE SPEC — read this first
    ├── QuantumChem Mobile v2.html     ← Final prototype (target visual)
    ├── QuantumChem Mobile.html        ← Earlier iteration (comparison only)
    ├── lib/
    │   ├── shared.jsx                 ← PhotoelectricScene + KEGraph SVG scenes
    │   └── dir-a-v2.jsx               ← Full prototype screen components
    └── assets/
        ├── logo.svg
        └── logo-dark.svg
```
